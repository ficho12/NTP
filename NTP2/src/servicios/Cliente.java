...
public class Cliente {
	...
	public static void main(String[] args) {
		...
		for(int i = 0; i < numServidores; i++)		//Para cada máquina
		{
			...
			URI uri=UriBuilder.fromUri("http://" + IP.get(i) + ":8080/NTP2/").build();	//"http://localhost:8080/NTP/"
			WebTarget target = client.target(uri);										//"http://" + Ip + ":8080/Servidor/"
			
			pares = new ArrayList<Par>();
			Par mejorPar=null;
			mejorPar = new Par(Long.MAX_VALUE,Long.MAX_VALUE);
	...
	static long determinarOffset(long t0, long t1, long t2, long t3) {
		System.out.println("t1: " + t1 + "  t0: " + t0+ "  t2: " + t2 + " t3: " + t3);
		return (t1-t0 + t2-t3)/(long)2;
	}
	...
	static Par marzullo(ArrayList<Par> pares) {
		...
		marzulloFin.delay = (marzullo.offset + marzullo.delay)/(long)2;
		marzulloFin.offset = marzullo.delay - (marzullo.offset + marzullo.delay)/(long)2;
